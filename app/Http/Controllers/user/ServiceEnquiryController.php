...
class ServiceEnquiryController extends Controller
{
    public function enquiry_list()
    {
        ...
        return view('user.service.enquiry.enquiry_list', ['enquiry' => $enquiry]);
    }
    ...
    public function add_enquiry()
    {
        ...
        return view('user.service.enquiry.add_enquiry', ['users' => $users, 'products' => $products, 'add_group' => $add_group]);
    }
    ...
    public function enquiry_store(Request $req)
    {
       ...
       $task_update = DB::table('service_task')->insert([
            'enq_id' => $insert_id,
            'enq_no' => $enq_no,
    ...
    public function view_enquiry($id)
    {
        ...
        $task = DB::table('service_task as t')
            ->leftJoin('users as u', 't.created_by', '=', 'u.id','u.id', 't.attended_by', '=', 'u.id')
            ->where('t.enq_id', $id)
        ...
        return view('user.service.enquiry.enquiry_view', [
            'enquiry' => $view_eq,
            'task' => $task,
            'user_id' => $user_id,
            'users' => $users,
    ...
    public function store_quote(Request $req)
    {
     ...
     $service_enq_update =   DB::table('service_enquiry')
            ->where('id', $req->enqid)
            ->update([
        ...
        return redirect()->route('user.service.enquiry.enquiry_view', ['id' => $req->enqid])->with([
            'status' => 'Success',
    ...
    public function enquiry_view($id)
    {
        ...
        $enquiry = DB::table('service_enquiry as enq')
            ->leftJoin('users as ur', 'enq.assign_to', '=', 'ur.id')
            ->leftJoin('products as pro', 'enq.product_category', '=', 'pro.id')
            ->where('enq.id', $id)
            ->select(
        ...
        return view('user.service.enquiry.enquiry_view', compact('enquiry', 'tasks'));
    }
    ...
    {
        $enquiries = DB::table('service_enquiry as eq')
            ->join('products as pt', 'eq.product_category', '=', 'pt.id')
            ->join('users as us', 'eq.assign_to', '=', 'us.id')
            ->leftJoin('products_group as pg', 'eq.enq_pro_group', '=', 'pg.id')
        ...
        return view('user.service.enquiry.byCycle', [
            'cycle'     => $cycle,
            'enquiries' => $enquiries,
        ]);
    }
